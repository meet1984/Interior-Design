...
const { generateInquiryAcknowledgementEmail, generateAdminInquiryNotificationEmail } = require('../utils/emailTemplates');
  ...
  try {
    ...
    if (productId) {
      const product = await db.Product.findByPk(productId, {
        include: [{ model: db.Collection, as: 'collection' }]
      });
      if (!product) {
        return res.status(400).json({ success: false, message: 'Target product not found' });
      }
      productName = product.name;
      collectionName = product.collection ? product.collection.name : 'N/A';
    }
    ...
    // 1. Send acknowledgement email to the customer (background)
    sendEmail({
      to: email,
      subject: 'Klare Homes: Inquiry Received',
      html: generateInquiryAcknowledgementEmail(name)
    ...
    // 3. Send notification to all admins (background)
    const adminEmails = admins.map(admin => admin.email);
    if (adminEmails.length > 0) {
      sendEmail({
        to: adminEmails.join(','),
        subject: `New Inquiry: ${subject}`,
        html: generateAdminInquiryNotificationEmail(name, email, subject, inquiryType || 'general', phone, productName, collectionName)
