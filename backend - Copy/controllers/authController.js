...
const { generateOtpEmail, generateWelcomeEmail } = require('../utils/emailTemplates');
  ...
  try {
    ...
    if (existingUser) {
      if (existingUser.status === 'unverified') {
        ...
        await sendEmail({
          to: email,
          subject: 'Verify Your Klare Homes Studio Account',
          html: generateOtpEmail(firstName, otpCode, 'Welcome to Klare Homes Studio! Please use the verification code below to activate your account.')
        });

        await logActivity(existingUser.id, 'user_re_registered', `Re-registered unverified account: ${existingUser.email}`, req);
...
const login = async (req, res) => {
  ...
  try {
    ...
    if (user.role.name === 'admin' || user.role.name === 'manager') {
      // Generate OTP
      await db.Otp.destroy({ where: { email, type: 'login' } });
      const otpCode = Math.floor(100000 + Math.random() * 900000).toString();
      const expiresAt = new Date(Date.now() + 10 * 60 * 1000); // 10 minutes
      ...
      await sendEmail({
        to: email,
        subject: 'Klare Homes Admin Login Verification',
        html: generateOtpEmail(user.firstName, otpCode, 'Verification is required to log into your administrative account.')
      });

      await logActivity(user.id, 'admin_login_otp_sent', `OTP sent to ${email} for login`, req);
...
const requestProfileUpdateOtp = async (req, res) => {
  try {
    ...
    await sendEmail({
      ...
      html: generateOtpEmail(user.firstName, otpCode, 'We received a request to update your profile. Please use the verification code below to confirm these changes.')
    });
...
const forgotPassword = async (req, res) => {
  ...
  try {
    ...
    await sendEmail({
      ...
      html: generateOtpEmail(user.firstName, otpCode, 'We received a request to reset your password. Please use the verification code below to proceed.')
    });
...
const verifyRegistrationOtp = async (req, res) => {
  ...
  try {
    ...
    await sendEmail({
      to: user.email,
      subject: 'Welcome to Klare Homes!',
...
const resendOtp = async (req, res) => {
  ...
  try {
    ...
    let contextMessage = 'Here is your new verification code:';

    if (type === 'registration') {
      subject = 'Verify Your Klare Homes Studio Account';
      contextMessage = 'Welcome! Here is your new verification code to activate your account:';
    } else if (type === 'forgot_password') {
      subject = 'Password Reset Request';
      contextMessage = 'Here is your new verification code to reset your password:';
    }

    await sendEmail({ 
      to: email, 
      subject, 
      html: generateOtpEmail(user.firstName, otpCode, contextMessage) 
    });
...
// Verify Login OTP (For Admin/Manager)
const verifyLoginOtp = async (req, res) => {
  ...
  try {
    ...
    // Enforce no "remember me" for admin/manager, always short lived token
    const token = generateTokens(user, false);

    await logActivity(user.id, 'user_login', `Logged in via 2FA from IP: ${req.ip}`, req);
