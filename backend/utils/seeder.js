...
const seedDatabase = async (db) => {
  try {
    ...
    const adminUser = await db.User.create({
      ...
      firstName: 'Elizabeth',
      lastName: 'Vance',
      email: 'admin@signature-interiors.com',
    ...
    const managerUser = await db.User.create({
      ...
      firstName: 'Julian',
      lastName: 'Kohl',
      email: 'manager@signature-interiors.com',
    ...
    const clientUser = await db.User.create({
      ...
      lastName: 'Müller',
      email: 'client@signature-interiors.com',
    ...
    await db.Setting.bulkCreate([
      { key: 'site_name', value: 'SIGNATURE Luxury Interiors', group: 'general' },
      { key: 'contact_email', value: 'concierge@signature-interiors.com', group: 'contact' },
    ...
    await db.Testimonial.bulkCreate([
      {
        ...
        avatar: '/uploads/client.webp',
        rating: 5,
        content: 'SIGNATURE transformed our kitchen into an architectural statement. The attention to detail is remarkable, matching the exact standards of luxury German engineering. The handleless drawers open with a gentle touch, and the material finish feels premium day after day.',
        status: 'approved'
      },
      {
        clientName: 'Dr. Andreas Fischer',
