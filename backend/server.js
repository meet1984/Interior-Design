...
if (process.env.NODE_ENV === 'production') {
  ...
  app.get('/', (req, res) => {
    res.json({
      status: 'online',
      system: 'SIGNATURE Luxury Interiors Management API',
      version: '1.0.0',
...
const startServer = async () => {
  try {
    ...
    // C. Sync database schema (in local development, creates/updates tables)
    await sequelize.sync({ alter: true }); // Automatically applies model changes
